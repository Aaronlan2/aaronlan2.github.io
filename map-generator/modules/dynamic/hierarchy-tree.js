...
export function open(props) {
  ...
  if (validElements.length < 3) return tip(`没有足够的${props.type}来显示层次结构`, false, "error");
  ...
  const html = /* html */ `<div id="hierarchyTree" class="dialog" style="overflow: hidden;">
    ...
    <div id="hierarchyTree_details" class='chartInfo'>
      ...
      <div id='hierarchyTree_selected' style="display: none">
        ...
        <span data-name="类型缩写">缩写：<input id='hierarchyTree_selectedCode' type='text' maxlength='3' size='3' /></span>
          <span>来源： <span id='hierarchyTree_selectedOrigins'></span></span>
        <button data-tip='编辑此节点的来源' class="hierarchyTree_selectedButton" id='hierarchyTree_selectedSelectButton'>编辑</button>
...
function getRoot() {
  try {
    ...
    tip("层次结构数据问题" + error, false, "error", 6000);
    return oldRoot;
  }
}
...
function selectElement(d) {
  ...
  byId("hierarchyTree_selectedCode").onchange = function () {
    if (this.value.length > 3) return tip("缩写不得超过3个字符", false, "error", 3000);
  ...
  const createOriginButtons = () => {
    byId("hierarchyTree_selectedOrigins").innerHTML = dataElement.origins
      ...
      .map((origin, index) => {
        ...
        const tip = `${type} origin: ${name}. 单击以删除指向该源的链接`;
  ...
  byId("hierarchyTree_selectedSelectButton").onclick = () => {
    ...
    const selectableElementsHtml = selectableElements.map(({i, name, code, color}) => {
      ...
      if (i === 0) {
        return /*html*/ `
        <div ${isChecked}>
          <input data-tip="设置为主源" type="radio" name="primary" value="${i}" ${isPrimary} />
          Top level
        </div>
      `;
      }
      ...
      return /*html*/ `
        <div ${isChecked}>
          ...
          <label data-tip="选中设置为次要源" for="selectElementOrigin${i}" class="checkbox-label">
