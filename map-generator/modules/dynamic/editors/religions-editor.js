...
function insertEditorHtml() {
  const editorHtml = /* html */ `<div id="religionsEditor" class="dialog stable">
    <div id="religionsHeader" class="header" style="grid-template-columns: 13em 6em 7em 18em 6em 7em 6em 7em">
      ...
      <div data-tip="点击按主神排序" class="sortable alphabetically hide" data-sortby="deity">主神&nbsp;</div>
      <div data-tip="点击按宗教区域排序" class="sortable hide" data-sortby="area">区域&nbsp;</div>
      <div data-tip="点击按信徒数量（宗教区域人口）排序" class="sortable hide" data-sortby="population">信徒&nbsp;</div>
      <div data-tip="点击按潜在影响范围类型排序" class="sortable alphabetically hide" data-sortby="expansion">潜力&nbsp;</div>
      <div data-tip="点击按扩张性排序" class="sortable hide" data-sortby="expansionism">扩张&nbsp;</div>
    </div>
    <div id="religionsBody" class="table" data-type="absolute"></div>

    <div id="religionsFooter" class="totalLine">
      <div data-tip="有组织宗教的总数" style="margin-left: 12px">
        有组织宗教：&nbsp;<span id="religionsOrganized">0</span>
      </div>
      <div data-tip="异端的总数" style="margin-left: 12px">
        异端：&nbsp;<span id="religionsHeresies">0</span>
      </div>
      <div data-tip="邪教的总数" style="margin-left: 12px">
        邪教：&nbsp;<span id="religionsCults">0</span>
      </div>
      <div data-tip="民间宗教的总数" style="margin-left: 12px">
    ...
    <div id="religionsBottom">
      ...
      <button id="religionsExtinct" data-tip="显示/隐藏已灭绝宗教（无单元格的宗教）" class="icon-eye-off"></button>
      ...
      <button id="religionsAdd" data-tip="添加新宗教。按住Shift可添加多个" class="icon-plus"></button>
      <button id="religionsExport" data-tip="下载与宗教相关的数据" class="icon-download"></button>
      <button id="religionsRecalculate" data-tip="根据当前增长相关属性值重新计算宗教" class="icon-retweet"></button>
      <span data-tip="允许宗教中心、影响范围和扩张性更改立即生效">
...
function religionsEditorAddLines() {
  ...
  for (const r of pack.religions) {
    ...
    const populationTip = `信徒：${si(population)}；农村地区：${si(rural)}；城市地区：${si(urban)}。点击以更改`;
    totalArea += area;
    ...
    if (!r.i) {
      ...
      >
        ...
        <span data-tip="点击重新生成主神" class="icon-arrows-cw placeholder hide"></span>
        <input data-tip="宗教主神" class="religionDeity placeholder hide" style="width: 17em" value="" autocorrect="off" spellcheck="false" />
        <span data-tip="宗教区域" style="padding-right: 4px" class="icon-map-o hide"></span>
        <div data-tip="宗教区域" class="religionArea hide" style="width: 6em">${si(area) + unit}</div>
    ...
    >
      ...
      <span data-tip="点击重新生成主神" class="icon-arrows-cw hide"></span>
      <input data-tip="宗教主神" class="religionDeity hide" style="width: 17em"
  ...
  byId("religionsOrganized").innerHTML = validReligions.filter(r => r.type === "组织").length;
  byId("religionsHeresies").innerHTML = validReligions.filter(r => r.type === "异端").length;
  byId("religionsCults").innerHTML = validReligions.filter(r => r.type === "教派").length;
  byId("religionsFolk").innerHTML = validReligions.filter(r => r.type === "民间").length;
...
function getExpansionColumns(r) {
  if (r.type === "民间") {
    const tip =
      "民间宗教不具有竞争性，也不会扩张。起初，它们覆盖其母文化的所有区域，但在有组织宗教扩张时会被排挤出去。";
    return /* html */ `
      <span data-tip="${tip}" class="icon-resize-full-alt hide" style="padding-right: 2px"></span>
      <span data-tip="${tip}" class="religionExtent hide" style="width: 5em">文化</span>
  ...
  return /* html */ `
    <span data-tip="潜在宗教影响范围" class="icon-resize-full-alt hide" style="padding-right: 2px"></span>
    <select data-tip="潜在宗教影响范围" class="religionExtent hide" style="width: 5em">
      ${getExtentOptions(r.expansion)}
    </select>
    <span data-tip="宗教扩张性。定义竞争规模" class="icon-resize-full hide"></span>
    <input
      data-tip="宗教扩张性。定义竞争规模。点击可更改，更改后点击重新计算以应用"
...
function changePopulation() {
  ...
  if (!religion.cells) return tip("宗教不存在于任何单元格中，不能改变人口", false, "error");
  ...
  alertMessage.innerHTML = /* html */ `<div>
    <i>宗教领地的所有人口都被视为该宗教的信徒。这意味着信徒数量的变化将直接影响人口数量</i>
...
async function showHierarchy() {
  ...
  const getDescription = religion => {
    ...
    const getTypeText = () => {
      ...
      if (type === "民间" || type === "组织") return `. ${type} religion`;
      return `. ${type}`;
    };
  ...
  const getShape = ({type}) => {
    if (type === "民间") return "circle";
    if (type === "组织") return "square";
    if (type === "教派") return "hexagon";
...
function addReligion() {
  ...
  if (pack.cells.h[center] < 20)
    return tip("你不能把宗教中心放在水中，请点击陆地单元格", false, "error");

  const occupied = pack.religions.some(r => !r.removed && r.center === center);
  if (occupied) return tip("这个单元格已经是另一个宗教的中心，请选择其他单元格", false, "error");
...
function downloadReligionsCsv() {
  ...
  const headers = `ID,名称,颜色,类型,政体,至高神,面积 ${unit},信徒数量,起源,潜力,扩张主义倾向`;  const lines = Array.from($body.querySelectorAll(":scope > div"));
