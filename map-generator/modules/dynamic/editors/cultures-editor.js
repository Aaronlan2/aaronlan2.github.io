...
const cultureTypes = ["通用", "河流", "湖泊", "航海", "游牧", "狩猎", "高原"];
...
function insertEditorHtml() {
  const editorHtml = /* html */ `<div id="culturesEditor" class="dialog stable">
    <div id="culturesHeader" class="header" style="grid-template-columns: 10em 7em 9em 4em 8em 5em 7em 8em">
      ...
      <div data-tip="点击按文化单元格数量排序" class="sortable hide" data-sortby="cells">单元格&nbsp;</div>
      <div data-tip="点击按扩张性排序" class="sortable hide" data-sortby="expansionism">扩张&nbsp;</div>
      <div data-tip="点击按文化区域排序" class="sortable hide" data-sortby="area">区域&nbsp;</div>
    ...
    <div id="culturesBottom">
      ...
      <button id="culturesAdd" data-tip="添加新文化。按住Shift可添加多个" class="icon-plus"></button>
      <button id="culturesExport" data-tip="下载与文化相关的数据" class="icon-download"></button>
      <button id="culturesImport" data-tip="上传与文化相关的数据" class="icon-upload"></button>
      <button id="culturesRecalculate" data-tip="根据当前增长相关属性值重新计算文化" class="icon-retweet"></button>
      <span data-tip="允许文化中心、扩张和类型更改立即生效">
  ...
  for (const c of pack.cultures) {
    ...
    if (!c.i) {
      ...
      lines += /* html */ `<div
        ...
        >
          ...
          <input data-tip="中性文化名称。单击并输入进行更改" class="cultureName italic" style="width: 7em"
          ...
          <span data-tip="点击为分配了此文化的城镇重新生成名称" class="icon-arrows-cw hide"></span>
          ...
          <span data-tip="文化区域" style="padding-right: 4px" class="icon-map-o hide"></span>
          <div data-tip="文化区域" class="cultureArea hide" style="width: 6em">${si(area)} ${unit}</div>
    ...
    lines += /* html */ `<div
      ...
      >
        ...
        <input data-tip="文化名称。点击并输入以更改" class="cultureName" style="width: 7em"
          value="${c.name}" autocorrect="off" spellcheck="false" />
        <span data-tip="重新生成文化名称" class="icon-cw hiddenIcon" style="visibility: hidden"></span>
        <select data-tip="文化类型。定义增长模式。点击可更改"
          class="cultureType">${getTypeOptions(c.type)}</select>
        <span data-tip="点击可为分配了此文化的城镇重新生成名称" class="icon-arrows-cw hide"></span>
        <select data-tip="文化名称库。点击可更改。点击箭头可重新生成名称"
          class="cultureBase">${getBaseOptions(c.base)}</select>
        <span data-tip="单元格数量" class="icon-check-empty hide"></span>
        <div data-tip="单元格数量" class="cultureCells hide" style="width: 4em">${c.cells}</div>
        <span data-tip="文化扩张性。定义竞争规模" class="icon-resize-full hide"></span>
        <input
          data-tip="文化扩张性。定义竞争规模。点击可更改，更改后点击重新计算以应用"
        ...
        <div data-tip="${populationTip}" class="culturePopulation hide pointer"
          style="width: 4em">${si(population)}</div>
        ${getShapeOptions(selectShape, c.shield)}
        <span data-tip="锁定文化" class="icon-lock${c.lock ? "" : "-open"} hide"></span>
...
function getShapeOptions(selectShape, selected) {
  ...
  return `<select data-tip="与文化相关的徽章形状。单击以更改" class="cultureEmblems hide">${options}</select>`;
}
...
function changePopulation() {
  ...
  if (!culture.cells) return tip("文化不存在于任何单元格中，不能改变人口", false, "error");
...
function cultureRegenerateBurgs() {
  ...
  tip(`城镇 ${cultureBurges.length} 的名称已重新生成`, false, "success");
}
...
function addCulture() {
  ...
  if (pack.cells.h[center] < 20)
    return tip("你不能把文化中心放在水里，请点击陆地单元格", false, "error");

  const occupied = pack.cultures.some(c => !c.removed && c.center === center);
  if (occupied) return tip("这个单元格已经是另一个文化的中心，请选择其他单元格", false, "error");
...
function downloadCulturesCsv() {
  ...
  const headers = `ID,名称,颜色,单元格,扩张主义倾向,类型,面积 ${unit},人口数量,名称库,徽标形状,起源`;
