...
function insertEditorHtml() {
  const editorHtml = /* html */ `<div id="statesEditor" class="dialog stable">
    <div id="statesHeader" class="header" style="grid-template-columns: 11em 8em 7em 7em 6em 6em 8em 6em 7em 6em">
      ...
      <div data-tip="点击按国家政体名称排序" class="sortable alphabetically" data-sortby="form">政体&nbsp;</div>
      ...
      <div data-tip="点击按国家城镇数量排序" class="sortable hide" data-sortby="burgs">城镇&nbsp;</div>
      ...
      <div data-tip="点击按国家单元格数量排序" class="sortable hidden show hide" data-sortby="cells">单元格&nbsp;</div>
    </div>
    ...
    <div id="statesFooter" class="totalLine">
      ...
      <div data-tip="陆地单元格总数" style="margin-left: 12px">单元格：&nbsp;<span id="statesFooterCells">0</span></div>
    ...
    <div id="statesBottom">
      ...
      <button id="statesRegenerate" data-tip="显示重新生成菜单和更多数据" class="icon-cog-alt"></button>
      <div id="statesRegenerateButtons" style="display: none">
        ...
        <button id="statesRecalculate" data-tip="根据当前增长相关属性值重新计算国家" class="icon-retweet"></button>
        <div data-tip="允许国家中立距离、扩张和类型更改立即生效" style="display: inline-block">
          <input id="statesAutoChange" class="checkbox" type="checkbox" />
          <label for="statesAutoChange" class="checkbox-label"><i>自动应用更改</i></label>
        </div>
        <div data-tip="当国家数据更改时允许系统更改国家标签" style="display: inline-block">
      ...
      <button id="statesAdd" data-tip="添加新国家。按住Shift可添加多个" class="icon-plus"></button>
      <button id="statesMerge" data-tip="将多个国家合并为一个" class="icon-layer-group"></button>
  ...
  for (const s of pack.states) {
    ...
    if (!s.i) {
      ...
      >
        ...
        <input data-tip="中立地区名称。点击可更改" class="stateName name pointer italic" value="${
          s.name
        }" readonly />
        ...
        <span data-tip="点击查看中立城镇" class="icon-dot-circled pointer hide" style="padding-right: 1px"></span>
        <div data-tip="城镇数量" class="stateBurgs hide">${s.burgs}</div>
        <span data-tip="中立地区面积" style="padding-right: 4px" class="icon-map-o hide"></span>
        ...
        <span data-tip="单元格数量" class="icon-check-empty ${hidden} show hide"></span>
        <div data-tip="单元格数量" class="stateCells ${hidden} show hide">${s.cells}</div>
      </div>`;
      continue;
    }
    ...
    >
      ...
      <input data-tip="国家名称。点击可更改" class="stateName name pointer" value="${s.name}" readonly />
      <svg data-tip="点击显示和编辑国家徽章" class="coaIcon pointer" viewBox="0 0 200 200"><use href="#stateCOA${
        s.i
      }"></use></svg>
      <input data-tip="国家政体名称。点击可更改" class="stateForm name pointer" value="${
        s.formName
      }" readonly />
      <span data-tip="国家首都。点击可缩放查看" class="icon-star-empty pointer"></span>
      <input data-tip="首都名称。点击并输入可重命名" class="stateCapital" value="${capital}" autocorrect="off" spellcheck="false" />
      <select data-tip="主导文化。点击可更改" class="stateCulture hide">${getCultureOptions(
        s.culture
      )}</select>
      <span data-tip="点击查看国家城镇" style="padding-right: 1px" class="icon-dot-circled pointer hide"></span>
      <div data-tip="城镇数量" class="stateBurgs hide">${s.burgs}</div>
      ...
      <span data-tip="${populationTip}" class="icon-male hide"></span>
      <div data-tip="${populationTip}" class="statePopulation pointer hide" style="width: 5em">${si(population)}</div>
      <select data-tip="国家类型。定义增长模式。点击可更改" class="cultureType ${hidden} show hide">${getTypeOptions(
      s.type
    )}</select>
      <span data-tip="国家扩张性" class="icon-resize-full ${hidden} show hide"></span>
      <input data-tip="扩张性（定义竞争规模）。更改后可基于新值重新计算国家"
      ...
      <span data-tip="锁定国家以防重新生成" class="icon-lock${
        s.lock ? "" : "-open"
      } hide"></span>
...
function changePopulation(stateId) {
  ...
  if (!state.cells) return tip("国家不存在于任何单元格中，不能改变人口", false, "error");
...
function addState() {
  ...
  if (cells.h[center] < 20)
    return tip("你不能把国家放在水里，请点击陆地单元格", false, "error");

  let burg = cells.burg[center];
  if (burg && burgs[burg].capital)
    return tip("这个单元格已经是另一个国家的首都，请选择其他单元格", false, "error");
  ...
  states[0].diplomacy.push([
    `独立宣言`,
    `${name} 宣布独立于 ${states[oldState].name}`
  ]);
      ...
      "合并": function () {
        ...
        confirmationDialog({
          ...
          message: /* html */ `
            ...
            <p>被移除国家的数据（城镇、省份、军团）将被分配给${emblem(rullingState.i)}${rullingState.name}。</p>
            <p>您确定要合并这些国家吗？此操作无法撤销。</p>`,
          confirm: "确定",
          onConfirm: () => {
...
function downloadStatesCsv() {
  ...
  const headers = `ID,国家,全称,政体,颜色,首都,文化,类型,扩张主义倾向,单元格,城镇数量,面积 ${unit},总人口,农村人口,城市人口`;
