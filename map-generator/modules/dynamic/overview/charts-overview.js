...
function insertHtml() {
  ...
  const html = /* html */ `<div id="chartsOverview" class="dialog stable">
    <form id="chartsOverview__form">
      <div>
        <button data-tip="添加图表" type="submit">绘图</button>

        <select data-tip="选择实体（Y轴）" id="chartsOverview__entitiesSelect">
          ${createOptions(entities)}
        </select>

        <label>按
          <select data-tip="选择绘图依据的值（X轴）" id="chartsOverview__plotBySelect">
            ${createOptions(plotBy)}
          </select>
        </label>

        <label>按以下分组
          <select data-tip="选择分组依据的实体。如果不需要分组，请将其设置为与实体相同" id="chartsOverview__groupBySelect">
            ${createOptions(entities)}
          </select>
        </label>

        <label data-tip="排序类型">排序方式
...
function renderChart({id, entity, plotBy, groupBy, sorting, type}) {
  ...
  const title = `${capitalize(entity)}按 ${plotByLabel}${noGrouping? "" : " 分组依据为 " + groupLabel}`;
...
function insertChart(id, sortedData, $chart, title) {
  ...
  $caption.innerHTML = /* html */ `
    ...
    <div>
      <button data-tip="将图表数据下载为文本文件（.csv）" class="icon-download"></button>
      <button data-tip="以svg格式下载图表（可在浏览器或Inkscape中打开）" class="icon-chart-bar"></button>
