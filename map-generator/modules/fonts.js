...
async function addGoogleFont(family) {
  ...
  if (!fontRanges) return tip("无法获取此值的谷歌字体", true, "error", 4000);
  tip(`谷歌字体 ${family} 正在加载中……`, true, "warn", 4000);
  ...
  Promise.all(promises)
    .then(fontFaces => {
      ...
      tip(`谷歌字体 ${family} 已添加至列表`, true, "success", 4000);
      addFontOption(family);
      document.getElementById("styleSelectFont").value = family;
      changeFont();
    })
    .catch(err => {
      tip(`载入谷歌字体 ${family} 失败`, true, "error", 4000);
      ERROR && console.error(err);
    });
}
...
function addLocalFont(family) {
  ...
  document.fonts.add(fontFace);
  tip(`本地字体 ${family} 已添加至列表`, true, "success", 4000);
  addFontOption(family);
  document.getElementById("styleSelectFont").value = family;
  changeFont();
}
...
function addWebFont(family, url) {
  ...
  document.fonts.add(fontFace);
  tip(`字体 ${family} 已添加至列表`, true, "success", 4000);
  addFontOption(family);
  document.getElementById("styleSelectFont").value = family;
  changeFont();
}
