...
async function exportToSvg() {
    ...
    const message = `${link.download}已保存。打开“下载”界面（Ctrl + J）查看`;
    ...
    img.onload = function () {
        ...
        canvas.toBlob(function (blob) {
            ...
            window.setTimeout(function () {
                ...
                const message = `${link.download}已保存。打开“下载”界面（Ctrl + J）查看。您可以在选项中设置图像比例`;
                tip(message, true, "success", 5000);
            }, 1000);
        });
    };
...
async function exportToJpeg() {
    ...
    img.onload = async function () {
        ...
        link.href = URL;
        link.click();
        tip(`${link.download}已保存。打开“下载”界面（CTRL + J）查看`, true, "success", 7000);
...
async function exportToPngTiles() {
    ...
    for (let y = 0, row = 0, id = 1; y + tileH <= graphHeight; y += tileH, row++) {
        ...
        for (let x = 0, cell = 1; x + tileW <= graphWidth; x += tileW, cell++, id++) {
            status.innerHTML = `渲染碎片中 ${rowName}${cell}（共${tolesTotal}个，当前第${id}个）...`;
    ...
    status.innerHTML = "压缩文件中...";
    zip.generateAsync({ type: "blob" }).then(blob => {
        status.innerHTML = "下载压缩包中...";
        const link = document.createElement("a");
        ...
        status.innerHTML = '完成。在“下载”中查看.zip文件（Ctrl + J）';
        setTimeout(() => URL.revokeObjectURL(link.href), 5000);
    });

    // 封装img.onload为Promise
        ...
        return new Promise((resolve, reject) => {
            canvas.toBlob(
                blob => {
                    ...
                    else reject(new Error("Canvas toBlob() 函数出错"));
                },
                mimeType,
                qualityArgument
            );
...
async function getMapURL(type, options) {
    ...
    const cloneEl = byId("map").cloneNode(true); // 克隆SVG
    ...
    if (customization && type === "mesh") updateMesh单元格(clone);
    inlineStyle(clone);
    ...
    if (!cloneEl.getElementById("fogging-cont")) cloneEl.getElementById("fog")?.remove(); // 移除未使用的雾气
    if (!cloneEl.getElementById("regions")) cloneEl.getElementById("statePaths")?.remove(); // 移除未使用的statePaths
    if (!cloneEl.getElementById("labels")) cloneEl.getElementById("textPaths")?.remove(); // 移除未使用的textPaths
...
function inlineStyle(clone) {
  ...
  clone.selectAll("g, #ruler *, #scaleBar > text").each(function () {
    ...
    for (let i = 0; i < compStyle.length; i++) {
      ...
      if (key === "cursor") continue; // 光标应设为默认
      if (this.hasAttribute(key)) continue; // 若存在相同属性，则不添加样式
...
function saveGeoJson单元格() {
  ...
  const fileName = getFileName("单元格") + ".geojson";
...
function saveGeoJsonRoutes() {
  ...
  const fileName = getFileName("路线") + ".geojson";
  downloadFile(JSON.stringify(json), fileName, "application/json");
}
...
function saveGeoJsonRivers() {
  ...
  const fileName = getFileName("河流") + ".geojson";
  downloadFile(JSON.stringify(json), fileName, "application/json");
}
...
function saveGeoJsonMarkers() {
  ...
  const fileName = getFileName("标记") + ".geojson";
  downloadFile(JSON.stringify(json), fileName, "application/json");
}