...
async function quickLoad() {
  ...
  if (blob) loadMapPrompt(blob);
    ...
    tip("未存储任何地图。先将地图保存到浏览器缓存", true, "error", 2000);
...
async function createSharableDropboxLink() {
  ...
  try {
    ...
    return tip("Dropbox API 出错。无法创建链接.", true, "error", 2000);
  }
}
...
function loadMapPrompt(blob) {
  ...
  alertMessage.innerHTML = /* html */ `您确定要加载保存的地图吗？<br />
...
function showUploadErrorMessage(error, URL, random) {
  ERROR && console.error(error);
  alertMessage.innerHTML = /* html */ `无法从 ${link(URL, "link provided")}加载地图 ${
    random ? `已随机生成新的地图` : ""
  } 请确保链接文件可访问，服务器端允许跨域访问`;
  $("#alert").dialog({
    title: "Loading error",
    width: "32em",
    buttons: {
...
async function parseLoadedData(data, mapVersion) {
  try {
    ...
    WARN && console.warn(`共计: ${rn((performance.now() - uploadMap.timeStart) / 1000, 2)}s`);
    showStatistics();
    INFO && console.groupEnd("加载地图 " + seed);
    tip("地图加载成功", true, "success", 7000);
  } catch (error) {
    ERROR && console.error(error);
    clearMainTip();

    alertMessage.innerHTML = /* html */ `地图加载时发生错误。请选择其他文件加载，<br>生成新的随机地图或取消加载。<br>地图版本：${mapVersion}。生成器版本：${VERSION}。
