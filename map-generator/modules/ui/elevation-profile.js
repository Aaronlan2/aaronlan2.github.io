...
// data是单元格索引数组，routeLen是距离（以实际米/英尺为单位），若是河流则isRiver应为true，否则为false
function showElevationProfile(data, routeLen, isRiver) {
    ...
    function downloadCSV() {
        let csv =
            "编号,x,y,纬度,经度,单元格,海拔高度,海拔数值,人口数量,城镇,城镇人口,生物群落,生物群落颜色,文化,文化颜色,宗教,宗教颜色,省份,省份颜色,州,州颜色\n"; // 表头
        ...
        const name = getFileName("海拔剖面图") + ".csv";
    ...
    function draw() {
        ...
        heightScale *= 0.9; // 曲线会使高度略微升高，在此处调整
        ...
        // 陆地
        let curve = d3.line().curve(d3.curveBasis); // 见 https://github.com/d3/d3-shape#curves
        ...
        for (let k = 0; k < chartData.points.length; k++) {
            if (chartData.burg[k] > 0) {
                ...
                let x1 = chartData.points[k][0]; // 默认图表左侧
                if (k > 0) x1 += xwidth / 2; // 若不是第一个则居中
                if (k == chartData.points.length - 1) x1 = chartWidth + xOffset; // 图表右侧
                y1 += add;
