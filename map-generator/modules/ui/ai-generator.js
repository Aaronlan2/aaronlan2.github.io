...
const SYSTEM_MESSAGE = "我正在制作我的架空地图。";
...
function generateWithAi(defaultPrompt, onApply) {
  ...
  $("#aiGenerator").dialog({
    ...
    buttons: {
      ...
      "应用": function () {
        ...
        if (!result) return tip("无结果可应用", true, "error", 4000);
        onApply(result);
        $(this).dialog("close");
      },
      "关闭": function () {
  ...
  async function generate(button) {
    ...
    if (!key) return tip("请输入API密钥", true, "error", 4000);
    ...
    if (isNaN(temperature)) return tip("温度必须为数字", true, "error", 4000);
