...
function editEmblem(type, id, el) {
  ...
  function upload(type) {
    ...
    if (file.size > 500000) {
      const message =
        "文件过大，请将文件大小优化至500kB以内并重新上传。推荐尺寸为200x200像素，大小不超过100kB";
    ...
    reader.onload = function (readerEvent) {
      ...
      let href = result; // 光栅图像
      if (type === "svg") {
        const el = document.createElement("html");
        el.innerHTML = result;

        el.querySelectorAll("*").forEach(el => {
          if (el.id === "adobe_illustrator_pgf") el.remove(); // 移除Adobe Illustrator内部数据

          el.getAttributeNames().forEach(attr => {
            // 移除sodipodi和inkscape属性
        ...
        if (!svg) {
          const message = "该文件不是有效的SVG。请使用Armoria或其他相关工具";
  ...
  async function download(format) {
    ...
    link.download = getFileName(`徽标 ${el.fullName || el.name}`) + "." + format;
  ...
  async function downloadGallery() {
    ...
    function runDownload() {
      ...
      const provinceSections = validStates
        .map(state => {
          ...
          return stateProvinces.length
            ? `<div id="provinces_${state.i}">${back}<h2>${state.fullName}的省份</h2>${figures}</div>`
            : "";
        })
        .join("");
      ...
      const burgSections = validStates
        .map(state => {
          ...
          let stateBurgSections = validProvinces
            ...
            .map(province => {
              ...
              return provinceBurgs.length
                ? `<div id="burgs_${province.i}">${back}<h2>${province.fullName}的城镇</h2>${provinceBurgFigures}</div>`
                : "";
            })
            .join("");
          ...
          if (stateBurgOutOfProvincesFigures)
            stateBurgSections += `<div><h2>${state.fullName}直接管辖的城镇</h2>${stateBurgOutOfProvincesFigures}</div>`;
      ...
      const FMG = `<a href="https://azgaar.github.io/Fantasy-Map-Generator" target="_blank">Azgaar的幻想地图生成器</a>`;
      const license = `<a target="_blank" href="https://github.com/Azgaar/Armoria#license">许可证</a>`;
      const html = /* html */ `<!DOCTYPE html>
        <html>
          <head>
            <title>${mapName.value}徽章图库</title>
          </head>
          <style type="text/css">
            body {
              margin: 0;
          ...
          <body>
            <div><h1>${mapName.value}徽章图库</h1></div>
            ${stateSection} ${provinceSections} ${burgSections} ${neutralsSection}
            <address>由${FMG}生成。该工具免费，但图片可能受版权保护，请参阅${license}</address>
          </body>
        </html>`;
