...
function overviewRoutes() {
  ...
  function routesOverviewAddLines() {
    ...
    for (const route of pack.routes) {
      ...
      >
        <span data-tip="点击以聚焦路线" class="icon-dot-circled pointer"></span>
        <div data-tip="路线名称" style="width: 15em; margin-left: 0.4em;">${route.name}</div>
        <div data-tip="路线组别" style="width: 8em;">${route.group}</div>
        <div data-tip="路线长度" style="width: 6em;">${length}</div>
  ...
  function triggerAllRoutesRemove() {
    alertMessage.innerHTML = /* html */ `确定要移除所有路线吗？此操作无法撤销`;
    $("#alert").dialog({
      resizable: false,
      title: "移除所有路线",
      buttons: {
