...
async function openEmblemEditor() {
  ...
  if (pack.states[1]?.coa) {
    ...
    tip("没有要编辑的徽章，请先生成国家和城镇", false, "error");
    return;
  }
...
function recreateStates() {
  ...
  if (!validBurgs.length) {
    tip("没有任何城镇可生成国家。请先创建城镇", false, "error");
    return null;
  }

  if (validBurgs.length < statesCount) {
    const message = `没有足够的城镇可生成 ${statesCount} 个国家。将仅生成 ${validBurgs.length} 个国家`;
  ...
  if (validStates.length && lockedStates.length === validStates.length) {
    tip("无法重新生成，所有国家都已被锁定", false, "error");
    return null;
  }
...
function regenerateEmblems() {
  ...
  pack.provinces.forEach(province => {
    ...
    if (!province.burg) {
      dominion = P(0.2);
      if (province.formName === "城市") dominion = P(0.95);
      else if (province.formName === "岛屿") dominion = P(0.6);
      else if (province.formName === "岛屿") dominion = P(0.5);
      else if (province.formName === "领土") dominion = P(0.4);
...
function regenerateZones(event) {
  if (isCtrlClick(event))
    prompt("请提供区域数量乘数", {default: 1, step: 0.01, min: 0, max: 100}, v =>
      addNumberOfZones(v)
    );
...
function toggleAddRiver() {
  ...
  tip("点击地图放置新河流或扩展现有河流。按住Shift放置多个河流", true, "warn");
...
function addRiverOnClick() {
  ...
  if (cells.r[i]) return tip("这里已经有河流了", false, "error");
  if (cells.h[i] < 20) return tip("无法在水单元格中创建河流", false, "error");
  if (cells.b[i]) return;
  ...
  while (i) {
    ...
    if (h[i] <= h[min]) return tip(`单元格 ${i} 地势较低，河流无法继续流动`, false, "error");
...
function configMarkersGeneration() {
  ...
  function drawConfigTable() {
    ...
    const headers = /* html */ `<thead style='font-weight:bold'><tr>
      <td data-tip="标记类型名称">Type</td>
      <td data-tip="标记图标">Icon</td>
      <td data-tip="标记数量乘数">Multiplier</td>
      <td data-tip="当前地图上该类型标记的数量">Number</td>
    </tr></thead>`;
