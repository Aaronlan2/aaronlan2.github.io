...
function overviewBurgs(settings = {stateId: null, cultureId: null}) {
  ...
  function burgsOverviewAddLines() {
    ...
    for (const b of filtered) {
      ...
      >
        <span data-tip="单击以放大视图" class="icon-dot-circled pointer"></span>
        <input data-tip="城镇的名字。单击以更改" class="burgName" value="${
          b.name
        }" autocorrect="off" spellcheck="false" />
        <input data-tip="城镇所在省份" class="burgState" value="${province}" disabled />
        <input data-tip="城镇所在国家" class="burgState" value="${state}" disabled />
        <select data-tip="主导文化。单击以更改城镇主导文化（使用文化编辑器更改单元格文化）" class="stateCulture">
          ${getCultureOptions(b.culture)}
        </select>
        <span data-tip="城镇人口" class="icon-male"></span>
        <input data-tip="城镇人口. Type to change" value=${si(
          population
        )} class="burgPopulation" style="width: 5em" />
        <div style="width: 3em">
          <span
            data-tip="${b.capital ? " 此城镇为国家首都" : "点击以重新分配国家首都"}"
            class="icon-star-empty${b.capital ? "" : " inactive pointer"}" style="padding: 0 1px;"></span>
          <span data-tip="单击以切换港口" class="icon-anchor pointer${
            b.port ? "" : " inactive"
  ...
  function changeBurgName() {
    if (this.value == "") tip("请提供一个名称", false, "error");
  ...
  function changeBurgPopulation() {
    ...
    if (this.value == "" || isNaN(+this.value)) {
      tip("请提供一个整数 (例如 10000，而非 10K 、10万)", false, "error");
  ...
  function triggerBurgRemove() {
    ...
    if (pack.burgs[burg].capital)
      return tip("你不能直接移除首都。请先迁都再执行此操作", false, "error");

    confirmationDialog({
      title: "移除城镇",
      message: "您确定要移除此城镇吗？<br>此操作无法恢复",
      confirm: "确定",
      onConfirm: () => {
        removeBurg(burg);
        burgsOverviewAddLines();
  ...
  function enterAddBurgMode() {
    ...
    tip("点击地图创建一个新城镇。按住Shift键添加多个", true, "warn");
  ...
  function addBurgOnClick() {
    ...
    if (pack.cells.h[cell] < 20)
      return tip("你不能把城镇放在水里。请点击陆地单元格", false, "error");
    if (pack.cells.burg[cell])
      return tip("这个单元格已经存在另一个城镇，请选择其他单元格", false, "error");
  ...
  function showBurgsChart() {
    ...
    if (data.length < 2) return tip("没有能显示的城镇", false, "error");
    ...
    function showInfo(ev, d) {
      ...
      burgsInfo.innerHTML = /* html */ `${name}. ${parent}. 人口: ${population}`;
      burgHighlightOn(ev);
      tip("单击以放大视图");
    }
  ...
  function downloadBurgsData() {
    let data = `ID,城镇,省份,省份全称,州,州全称,文化,宗教,人口数量,X坐标,Y坐标,纬度,经度,海拔 (${heightUnit.value}),温度,温度相似度,首都,港口,城堡,城墙,广场,神庙,棚户区,徽标,城市生成器链接\n`; // 表头
    ...
    const name = getFileName("城镇") + ".csv";
    downloadFile(data, name);
  }

  function renameBurgsInBulk() {
    alertMessage.innerHTML = /* html */ `将城镇列表下载为文本文件，进行修改后重新上传。请确保该文件为纯文本文档，每个名称单独占一行（分隔符为CRLF）。如果不想更改名称，保持原样即可。`;
      ...
      buttons: {
        "下载": function () {
          const data = pack.burgs
            ...
            .map(b => b.name)
            .join("\r\n");
          const name = getFileName("城镇名称") + ".txt";
          downloadFile(data, name);
        },
  ...
  function importBurgNames(dataLoaded) {
    if (!dataLoaded) return tip("无法加载文件，请检查文件格式是否正确", false, "error");
    const data = dataLoaded.split("\r\n");
    if (!data.length) return tip("无法解析列表，请检查文件格式是否正确", false, "error");
    ...
    if (!change.length) message = "文件中未发现任何更改。请更改一些名称以获得结果";
  ...
  function triggerAllBurgsRemove() {
    ...
    confirmationDialog({
      title: `移除 ${number} 个城镇`,
      message: `您确定要删除除首都外的所有<i>未锁定</i>的城镇吗？<br><i> 要移除一个首都，你必须先移除一个国家</i>`,
      confirm: "确定",
