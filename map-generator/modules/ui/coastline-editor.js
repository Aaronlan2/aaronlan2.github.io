...
function editCoastline() {
  ...
  function drawCoastlineVertices() {
    ...
    debug
      ...
      .on("mousemove", () =>
        tip("拖动以移动顶点，此功能仅用于微调。编辑高程图以更改实际单元格高度！")
      );
  ...
  function createNewGroup() {
    if (!this.value) return tip("请提供有效的组名");
    ...
    if (byId(group)) return tip("具有此id的元素已存在。请提供一个唯一的名称", false, "error");
  ...
  function removeCoastlineGroup() {
    ...
    alertMessage.innerHTML = /* html */ `您确定要删除该组吗？该组的所有海岸线元素（${count}）将被移至 <i>sea_island</i> group`;
    $("#alert").dialog({
      resizable: false,
      title: "移除海岸线组",
      width: "26em",
