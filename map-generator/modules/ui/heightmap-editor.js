...
function editHeightmap(options) {
  ...
  function showModeDialog() {
    alertMessage.innerHTML = /* html */ `高程图是所有其他数据（河流、城镇、国家等）所基于的核心元素。因此，最佳编辑方法是
    <i>擦除</i>次要数据，并在编辑完成后让系统自动重新生成这些数据。
    <p><i>擦除</i>模式还允许您将图像转换为高程图或使用模板编辑器。</p>
    <p>您可以<i>保留</i>数据，但您将无法更改海岸线。</p>
    <p>尝试<i>风险</i>模式以更改海岸线并保留数据。数据将会尽可能恢复，但这可能会导致不可预测的错误。</p>
    <p>请在编辑高程图之前<span class="pseudoLink" onclick="saveMap('machine')">保存地图</span>！</p>
    <p style="margin-bottom: 0">查阅${link(
    ...
    $("#alert").dialog({
      ...
      width: "28em",
      buttons: {
        擦除数据: () => enterHeightmapEditMode("erase"),
        保留数据: () => enterHeightmapEditMode("keep"),
        风险模式: () => enterHeightmapEditMode("risk"),
        取消: function () {
          $(this).dialog("close");
        }
      }
  ...
  function enterHeightmapEditMode(mode) {
    ...
    customization = 1;
    closeDialogs();
    tip('高程图处于编辑模式。点击“退出自定义模式”以完成高程图编辑', true);
  ...
  function finalizeHeightmap() {
    if (viewbox.select("#heights").selectAll("*").size() < 200)
      return tip("土地面积不足，应该至少有200个陆地单元格！", null, "error");
  ...
  function openBrushesPanel() {
    ...
    function placeLinearFeature() {
      ...
      if (power === 0) return tip("强度不应为0", false, "error");
    ...
    function changeOnlyLandClick(e) {
      ...
      e.preventDefault();
      tip("您无法在“保持”编辑模式下更改海岸线", false, "error");
    }
    ...
    function rescaleWithCondition() {
      ...
      if ((operator === "add" || operator === "subtract") && !Number.isInteger(operand))
        return tip("操作数应该是一个整数", false, "error");
    ...
    function startFromScratch() {
      if (changeOnlyLand.checked) return tip("设置“仅更改陆地单元”模式时不允许该操作", false, "error");
      const someHeights = grid.cells.h.some(h => h);
      if (!someHeights)
        return tip("高程图已清除，非必要请不要单击两次", false, "error");
  ...
  function openTemplateEditor() {
    ...
    function getStepHTML(type, count, arg3, arg4, arg5) {
      ...
      const TempY = /* html */ `<span>y:
          <input class="templateY" data-tip="沿Y轴的放置范围百分比（最小Y-最大Y）" value=${
            arg5 || "20-80"
          } />
        </span>`;

      const TempX = /* html */ `<span>x:
          <input class="templateX" data-tip="沿X轴的放置范围百分比（最小X-最大X）" value=${
            arg4 || "15-85"
          } />
        </span>`;

      const Height = /* html */ `<span>h:
          <input class="templateHeight" data-tip="区块最大高度，使用连字符获取范围内的随机数" value=${
            arg3 || "40-50"
          } />
        </span>`;

      const Count = /* html */ `<span>n:
          <input class="templateCount" data-tip="要添加的区块数量，使用连字符获取范围内的随机数" value=${
            count || "1-2"
          } />
        </span>`;
      ...
      if (type === "Strait")
        return /* html */ `${common}
          ...
          <span>w:
            <input class="templateCount" data-tip="海峡宽度，使用连字符获取范围内的随机数" value=${
              count || "2-7"
            } />
          </span>
        </div>`;
      ...
      if (type === "Invert")
        return /* html */ `${common}
          <span>by:
            <select class="templateDist" data-tip="沿轴镜像高程图" style="width: 7.8em">
              <option value="x" selected>x轴</option>
              <option value="y">y轴</option>
              <option value="xy">双轴</option>
            </select>
          </span>
          <span>n:
            <input class="templateCount" data-tip="反转概率，范围0-1" value=${count || "0.5"} />
          </span>
        </div>`;
        ...
        return /* html */ `${common}
          <span>f:
            <input class="templateCount"
              data-tip="设置遮罩比例。1-完全隔离（防止地图边缘出现陆地），2-半隔离等。负数表示反转效果"
      ...
      if (type === "Add")
        return /* html */ `${common}
          ...
          <span>v:
            <input class="templateCount" data-tip="向所有单元格的高度添加值（允许负值）"
      ...
      if (type === "Multiply")
        return /* html */ `${common}
          ...
          <span>v:
            <input class="templateCount" data-tip="将所有单元格的高度乘以该值" type="number" 
      ...
      if (type === "Smooth")
        return /* html */ `${common}
          <span>f:
            <input class="templateCount" data-tip="设置平滑比例。1-完全平滑，2-半平滑等" 
    ...
    function setRange(event) {
      ...
      prompt("设置高度间隔。避免空格，使用连字符作为分隔符", {default: "17-20"}, v => {
        const opt = document.createElement("option");
    ...
    function selectTemplate(e) {
      ...
      alertMessage.innerHTML = "您确定要选择其他模板吗？所有更改都将丢失";
      $("#alert").dialog({
        resizable: false,
        title: "更换模板",
        buttons: {
    ...
    function changeTemplate(template) {
      ...
      if (!steps.length) return tip(`高程图模板：未定义步骤`, false, "error");
    ...
    function uploadTemplate(dataLoaded) {
      ...
      for (const s of steps) {
        const step = s.split(" ");
        if (step.length !== 5) {
          ERROR && console.error("无法解析步骤，参数计数错误", s);
          continue;
        }
  ...
  function openImageConverter() {
    ...
    setOverlayOpacity(0);
    clearMainTip();
    tip("图像转换器已打开。上传图像并为每种颜色指定高度值", false, "warn"); // main tip
    ...
    function autoAssing(type) {
      ...
      if (!unassigned.length) {
        ...
        if (!unassigned.length)
          return tip("没有未指定的颜色。请加载图像并再次单击按钮", false, "error");
      }
    ...
    function setConvertColorsNumber() {
      prompt(
        `请设置最大颜色数。<br>实际数字通常较低，具体取决于配色方案`,
    ...
    function applyConversion() {
      if (colorsAssignedContainer.childElementCount < 3)
        return tip("请先为不同高度指定颜色", false, "error");
    ...
    function closeImageConverter(event) {
      ...
      alertMessage.innerHTML = /* html */ `您确定要关闭图像转换器吗？单击“取消”继续当前进度。点击“完成”直接应用此转换。点击“关闭”退出转换模式并恢复之前的高程图`;
  ...
  function toggleHeightmapPreview() {
    ...
    preview.on("mouseover", () => tip("高程图预览。点击下载屏幕大小的图像"));
  ...
  function downloadPreview() {
    ...
    img.onload = function () {
      ...
      link.download = getFileName("高程图") + ".png";
      link.href = imgBig;
      link.click();
      canvas.remove();
    };
