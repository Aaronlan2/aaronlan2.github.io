...
function editZones() {
  ...
  body.on("click", function (ev) {
    ...
    if (ev.target.closest("fill-box"))
      changeFill(ev.target.closest("fill-box").getAttribute("fill"), zone);
    else if (ev.target.classList.contains("zonePopulation"))
    ...
    else if (ev.target.classList.contains("zoneFog"))
      toggleFog(zone, ev.target.classList);
  });
  ...
  body.on("input", function (ev) {
    ...
    if (ev.target.classList.contains("zoneName"))
      changeDescription(zone, ev.target.value);
    else if (ev.target.classList.contains("zoneType"))
      changeType(zone, ev.target.value);
  });
  ...
  function updateFilters() {
    ...
    const typeToFilterBy = types.includes(zonesFilterType.value)
      ? zonesFilterType.value
      : "all";
  ...
  function zonesEditorAddLines() {
    ...
    const filteredZones =
      typeToFilterBy === "all"
        ? pack.zones
        : pack.zones.filter((zone) => zone.type === typeToFilterBy);

    const lines = filteredZones.map(
      ({ i, name, type, cells, color, hidden }) => {
        const area = getArea(d3.sum(cells.map((i) => pack.cells.area[i])));
        const rural =
          d3.sum(cells.map((i) => pack.cells.pop[i])) * populationRate;
        const urban =
          d3.sum(
            cells
              .map((i) => pack.cells.burg[i])
              .map((b) => pack.burgs[b].population)
          ) *
          populationRate *
          urbanization;
        ...
        data-type="${type}" data-cells=${
          cells.length
        } data-area=${area} data-population=${population} style="${
          hidden && "opacity: 0.5"
        }">
        <fill-box fill="${color}"></fill-box>
        <input data-tip="区域描述。点击并输入以修改" style="width: 11em" class="zoneName" value="${name}" autocorrect="off" spellcheck="false">
        <input data-tip="区域类型。点击并输入以修改" class="zoneType" value="${type}">
        <span data-tip="单元格数量" class="icon-check-empty hide"></span>
        <div data-tip="单元格数量" class="stateCells hide">${cells.length}</div>
        <span data-tip="区域面积" style="padding-right:4px" class="icon-map-o hide"></span>
        <div data-tip="区域面积" class="biomeArea hide">${
          si(area) + " " + getAreaUnit()
        }</div>
        <span data-tip="${populationTip}" class="icon-male hide"></span>
        <div data-tip="${populationTip}" class="zonePopulation hide pointer">${si(
          population
        )}</div>
        <span data-tip="拖动以提升或降低区域" class="icon-resize-vertical hide"></span>
        <span data-tip="切换区域焦点" class="zoneFog icon-pin ${
          focused ? "" : "inactive"
        } hide ${cells.length ? "" : "placeholder"}"></span>
    ...
    const totalPop =
      (d3.sum(pack.cells.pop) +
        d3.sum(pack.burgs.filter((b) => !b.removed).map((b) => b.population)) *
          urbanization) *
      populationRate;
    zonesFooterPopulation.dataset.population = totalPop;
    zonesFooterNumber.innerHTML = `${filteredZones.length} / ${pack.zones.length}`;
    ...
    body
      ...
      .forEach((el) => el.on("mouseenter", zoneHighlightOn));
    body
      .querySelectorAll("div.states")
      .forEach((el) => el.on("mouseleave", zoneHighlightOff));
  ...
  function enterZonesManualAssignent() {
    ...
    body
      .querySelectorAll("div > input, select, svg")
    ...
    const visibleZones = pack.zones.filter(
      (zone) => !zone.hidden && (!isFiltered || zone.type === filterBy)
    );
    const data = visibleZones
      .map(({ i, cells, color }) =>
        cells.map((cell) => ({ cell, zoneId: i, fill: color }))
      )
      .flat();
    zones
      .selectAll("polygon")
  ...
  function toggleLegend() {
    ...
    const data = visibleZones.map(({ i, name, color }) => [
      "zone" + i,
      color,
      name,
    ]);
  ...
  function togglePercentageMode() {
    if (body.dataset.type === "absolute") {
      ...
      body.querySelectorAll(":scope > div").forEach(function (el) {
        el.querySelector(".stateCells").innerHTML =
          rn((+el.dataset.cells / totalCells) * 100, 2) + "%";
        el.querySelector(".biomeArea").innerHTML =
          rn((+el.dataset.area / totalArea) * 100, 2) + "%";
        el.querySelector(".zonePopulation").innerHTML =
  ...
  function addZonesLayer() {
    const zoneId = pack.zones.length
      ? Math.max(...pack.zones.map((z) => z.i)) + 1
      : 0;
    const name = "未知区域";
    const type = "未知";
  ...
  function downloadZonesData() {
    const unit =
      areaUnit.value === "square"
        ...
        : areaUnit.value;
    let data = "ID,颜色,描述,类型,单元格数量,面积 " + unit + ",人口\n"; // headers
  ...
  function changePopulation(zone) {
    ...
    if (!landCells.length)
      return tip("区域没有陆地单元格，无法更改人口", false, "error");

    const burgs = pack.burgs.filter(
      (b) => !b.removed && landCells.includes(b.cell)
    );
    const rural = rn(
      d3.sum(landCells.map((i) => pack.cells.pop[i])) * populationRate
    );
    const urban = rn(
      d3.sum(
        landCells
    ...
    function applyPopulationChange() {
      ...
      if (isFinite(urbanChange) && urbanChange !== 1) {
        burgs.forEach(
          (b) => (b.population = rn(b.population * urbanChange, 4))
        );
      }
