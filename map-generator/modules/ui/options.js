...
function mapSizeInputChange() {
  ...
  if (tooWide || tooHigh) {
    const message = `画布大小大于窗口大小 (${window.innerWidth} x ${window.innerHeight})。这可能会影响性能`;
...
function generateMapWithSeed() {
  if (optionsSeed.value === seed) return tip("当前地图已使用此种子", false, "error");
...
function showSeedHistoryDialog() {
  const lines = mapHistory.map((h, i) => {
    ...
    return `<li>种子: ${h.seed} ${button}. 尺寸: ${h.width}x${h.height}. 模板: ${h.template}. 创建时间: ${created}</li>`;
  });
...
function copyMapURL() {
  const locked = document.querySelectorAll("i.icon-lock").length; // 检查是否有选项被锁定
  ...
  navigator.clipboard
    ...
    .then(() => {
      tip("地图URL已复制到剪贴板", false, "success", 3000);
      //window.history.pushState({}, null, search);
    })
    .catch(err => tip("无法复制URL: " + err, false, "error", 5000));
}
...
function generateEra() {
  ...
  if (!stored("era")) eraInput.value = Names.getBaseShort(P(0.7) ? 1 : rand(nameBases.length)) + "时代";
...
function regenerateEra() {
  unlock("era");
  options.era = eraInput.value = Names.getBaseShort(P(0.7) ? 1 : rand(nameBases.length)) + "时代";
  options.eraShort = options.era
    .split(" ")
    .map(w => w[0].toUpperCase())
    .join("");
...
function regeneratePrompt(options) {
  if (customization)
    return tip("处于编辑模式时无法生成新地图，请退出该模式后重试", false, "error");
  const workingTime = (Date.now() - last(mapHistory).created) / 60000; // 分钟
...
async function showLoadPane() {
  ...
  if (Cloud.providers.dropbox.api) {
    ...
    if (!files) {
      ...
      fileSelect.innerHTML = /* html */ `<option value="" disabled selected>请先将文件保存到Dropbox</option>`;
      return;
    }
...
function openExportToPngFragments() {
...
async function enter3dView(type) {
  ...
  canvas.onmouseenter = () => {
    const help =
      "左键改变角度，中键。鼠标滚轮缩放。右键或按住Shift键平移。<b>O</b>切换选项";
