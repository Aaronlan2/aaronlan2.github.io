...
function editBurg(id) {
  ...
  function dragBurgLabel() {
    ...
    d3.event.on("drag", function () {
      ...
      tip('使用拖动进行微调，如果需要实际移动城镇所在单元格，请使用“重新定位”按钮', false, "warning");
    });
  }
  ...
  function createNewGroup() {
    ...
    if (byId(group)) {
      tip("具有此id的元素已存在。请提供一个唯一的名称", false, "error");
      return;
    }
    ...
    if (!label || !icon) {
      ERROR && console.error("找不到标签或图标元素");
      return;
    }
  ...
  function removeBurgsGroup() {
    ...
    confirmationDialog({
      title: "移除城镇组",
      message: `你确定要移除 ${
        basic || capital? "城镇组中所有已解锁的元素" : "整个城镇组"
      }吗？<br />请注意，首都城镇或锁定的城镇不会被删除。 <br /><br />即将移除的城镇数量: ${
  ...
  function changeBurgLink() {
    ...
    prompt(
      "为城镇地图提供自定义链接。它可以是指向“中世纪奇幻城市生成器”的链接、其他工具的链接，或者仅仅是一张图片的链接。若留空则使用默认地图。",
  ...
  function toggleRelocateBurg() {
    ...
    if (byId("burgRelocate").classList.contains("pressed")) {
      ...
      tip("点击地图重新设置城镇位置。按住Shift键可连续移动", true);
  ...
  function relocateBurgOnClick() {
    ...
    if (cells.h[cell] < 20) {
      tip("不能把城镇放进水里，请选择陆地单元格", false, "error");
      return;
    }

    if (cells.burg[cell] && cells.burg[cell] !== id) {
      tip("这个单元格里已经有一个城镇了，请选择其他单元格", false, "error");
      return;
    }
  ...
  function removeSelectedBurg() {
    ...
    if (pack.burgs[id].capital) {
      alertMessage.innerHTML = /* html */ `你不能移除这个城镇，因为它是国家首都。<br/><br/>
您可以使用城镇编辑器（shift+T）更改首都`;
      $("#alert").dialog({
        resizable: false,
        title: "移除城镇",
        buttons: {
