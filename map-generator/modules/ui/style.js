...
openCreateHeightmapSchemeButton.on("click", function () {
  ...
  alertMessage.innerHTML = /* html */ `<div>
    <i>定义高程图渐变颜色从高到低海拔</i>
    <img id="heightmapSchemePreview" alt="高程图预览" style="margin-top: 0.5em; width: 100%;" />
  ...
  function renderStops() {
    ...
    const removeStopButton = index =>
      `<button class="remove" data-index="${index}" data-tip="移除颜色点" style="margin-top: 0.3em; height: max-content;">x</button>`;
    const addStopButton = () =>
      `<button class="add" data-tip="在中间添加颜色点" style="margin-top: 0.3em; height: max-content;">+</button>`;
  ...
  $("#addFontDialog").dialog({
    ...
    buttons: {
      添加: function () {
        ...
        if (!family) return tip("请输入字体名称", false, "error");

        const existingFont =
          method === "fontURL"
            ? fonts.find(font => font.family === family && font.src === src)
            : fonts.find(font => font.family === family);
        if (existingFont) return tip("此字体已被添加", false, "error");
...
function changeIconSize(size, group) {
  ...
  if (!el.size()) {
    console.warn(`组 ${group} 未找到。无法设置图标大小！`);
    return;
  }
...
function textureProvideURL() {
  alertMessage.innerHTML = /* html */ `提供自定义纹理图像 URL：
  ...
  $("#alert").dialog({
    ...
    buttons: {
      应用: function () {
        if (!textureURL.value) return tip("请输入有效 URL", false, "error");
