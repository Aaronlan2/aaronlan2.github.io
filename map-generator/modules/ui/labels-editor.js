...
function editLabel() {
  ...
  function showEditorTips() {
    showMainTip();
    if (d3.event.target.parentNode.parentNode.id === elSelected.attr("id")) tip("拖动标签");
    else if (d3.event.target.parentNode.id === "controlPoints") {
      if (d3.event.target.tagName === "circle") tip("拖动控制点，点击删除");
      if (d3.event.target.tagName === "path") tip("点击添加控制点");
    }
  }
  ...
  function createNewGroup() {
    ...
    if (byId(group)) {
      tip("此ID已存在。请提供唯一名称", false, "error");
      return;
    }

    if (Number.isFinite(+group.charAt(0))) {
      tip("名称应以字母开头", false, "error");
      return;
    }
  ...
  function removeLabelsGroup() {
    ...
    alertMessage.innerHTML = /* html */ `确定要删除${
      basic ? "组中的所有元素" : "整个标签组"
    }吗？ <br /><br />将删除的标签: ${count}`;
    $("#alert").dialog({
      resizable: false,
      title: "删除标签组",
      buttons: {
  ...
  function changeText() {
    ...
    if (elSelected.attr("id").slice(0, 10) === "stateLabel")
      tip("使用州编辑器更改实际州名，而不是标签", false, "warning");
  }
  ...
  function removeLabel() {
    alertMessage.innerHTML = "确定要删除标签吗？";
    $("#alert").dialog({
      resizable: false,
      title: "删除标签",
      buttons: {
