...
function editIce() {
  ...
  const type = elSelected.attr("type") ? "冰川" : "冰山";
  document.getElementById("iceRandomize").style.display = type === "冰川" ? "none" : "inline-block";
  document.getElementById("iceSize").style.display = type === "冰川" ? "none" : "inline-block";
  if (type === "冰山") document.getElementById("iceSize").value = +elSelected.attr("size");
  ...
  function toggleAdd() {
    ...
    if (document.getElementById("iceNew").classList.contains("pressed")) {
      ...
      tip("点击地图创建" + type + "。按住Shift键添加多个", true);
    } else {
      clearMainTip();
  ...
  function removeIce() {
    ...
    alertMessage.innerHTML = /* html */ `您确定要移除这个 ${type} 吗？`;
    $("#alert").dialog({
      resizable: false,
      title: "移除" + type,
      buttons: {
