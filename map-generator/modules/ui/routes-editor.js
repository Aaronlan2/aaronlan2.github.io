...
function editRoute(id) {
  ...
  tip(
    "拖动控制点以更改路线。点击点以删除它。点击路线以添加额外的控制点。对于主要更改，请创建新路线",
    true
  );
  ...
  function addControlPoint() {
    ...
    if (isNewCell) {
      ...
      if (!prev) ERROR && console.error("不能在路线开头添加控制点");
      if (!next) ERROR && console.error("不能在路线末尾添加控制点");
      if (!prev || !next) return;
  ...
  function openJoinRoutesDialog() {
    ...
    if (candidateRoutes.length) {
      ...
      alertMessage.innerHTML = /* html */ `<div>要加入的路线:
      ...
      tip("没有路线可加入。路线必须以当前路线的开始或结束单元格开始或结束", false, "error", 4000);
    }
  }
