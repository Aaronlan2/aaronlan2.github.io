...
function overviewMarkers() {
        ...
        const lines = pack.markers
          .map(({i, type, icon, pinned, lock}) => {
              ...
              <div class="states" data - i=${i} data - type="${type}">
                ...
                <span style="padding - right:.1em" data - tip="聚焦到标记位置" class="icon - dot - circled pointer"></span>
                <span style="padding - right:.1em" data - tip="固定标记（仅显示固定的标记）" class="icon - pin ${
                    pinned? "" : "inactive"
                }" pointer></span>
    ...
    function triggerRemoveAll() {
        confirmationDialog({
            title: "移除所有标记",
            message: "你确定要移除所有未锁定的标记吗？此操作无法撤销",
            confirm: "全部移除",
    ...
    function exportMarkers() {
        const headers = "编号,类型,图标,名称,备注,X坐标,Y坐标,纬度,经度\n";
        ...
        const body = pack.markers.map(marker => {
            ...
            const name = note? quote(note.name) : "未知";
        ...
        const fileName = getFileName("标记") + ".csv";
