...
function editUnits() {
  ...
  function changeDistanceUnit() {
    if (this.value === "custom_name") {
      prompt("为距离单位提供自定义名称", {default: ""}, custom => {
  ...
  function toggleOpisometerMode() {
    if (this.classList.contains("pressed")) {
      ...
      if (!layerIsOn("toggleRulers")) toggleRulers();
      tip("绘制曲线以测量长度。按住Shift键以禁用路径优化", true);
  ...
  function toggleRouteOpisometerMode() {
    if (this.classList.contains("pressed")) {
      ...
      if (!layerIsOn("toggleRulers")) toggleRulers();
      tip("绘制曲线沿路线测量长度。按住Shift键测量远离道路的距离", true);
      unitsBottom.querySelectorAll(".pressed").forEach(button => button.classList.remove("pressed"));
      this.classList.add("pressed");
  ...
  function togglePlanimeterMode() {
    if (this.classList.contains("pressed")) {
      ...
      if (!layerIsOn("toggleRulers")) toggleRulers();
      tip("绘制曲线测量其面积。按住Shift键禁用路径优化", true);
      unitsBottom.querySelectorAll(".pressed").forEach(button => button.classList.remove("pressed"));
      this.classList.add("pressed");
      viewbox.style("cursor", "crosshair").call(
        d3.drag().on("start", function () {
  ...
  function removeAllRulers() {
    ...
    alertMessage.innerHTML = /* html */ ` 确定要删除所有已放置的标尺吗？
      <br />如果您只想隐藏标尺，请在菜单中切换标尺图层关闭`;
    $("#alert").dialog({
      resizable: false,
      title: "删除所有标尺",
      buttons: {
