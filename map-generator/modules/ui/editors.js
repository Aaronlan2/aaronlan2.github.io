...
function moveBurgToGroup(id, g) {
  ...
  if (!label || !icon) {
    ERROR && console.error(`找不到 ID ${id} 的标签或图标元素`);
    return;
  }
...
function toggleCapital(burgId) {
  ...
  if (burgs[burgId].capital)
    return tip("要更换首都，请将首都迁移到给该国家的另一个城镇", false, "error");

  const stateId = burgs[burgId].state;
  if (!stateId) return tip("中立地区不能拥有首都", false, "error");
...
function togglePort(burg) {
  ...
  if (!haven) tip("未找到港口，系统将无法指定航线", false, "warn");
  b.port = port;
...
function createPicker() {
  ...
  const spaces = picker
    ...
    .attr("width", 303)
    .attr("height", 20)
    .on("mousemove", () => tip("不同颜色空间中的颜色值。编辑以更改"));
...
function selectIcon(initial, callback) {
  ...
  table.onmouseover = e => {
    if (e.target.tagName === "TD") tip(`单击以选择${e.target.textContent}图标`);
  };
  ...
  byId("addImage").onclick = function () {
    ...
    if (!ulr) return tip("输入要添加的图像URL", false, "error", 4000);
