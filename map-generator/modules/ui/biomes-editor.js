...
function editBiomes() {
  ...
  function biomesEditorAddLines() {
    ...
    for (const i of b.i) {
      if (!i || biomesData.name[i] === "removed") continue; // 忽略水域和已移除的生物群系
      ...
      lines += /* html */ `
        ...
        >
          ...
          <input data-tip="生物群系名称。点击可修改" class="biomeName" value="${
            b.name[i]
          }" autocorrect="off" spellcheck="false" />
          <span data-tip="生物群系宜居度百分比" class="hide">%</span>
          <input
            data-tip="生物群系宜居度百分比。点击可设置新值"
            type="number"
            min="0"
            max="9999"
          ...
          <div data-tip="单元格数量" class="biomeCells hide">${b.cells[i]}</div>
          ...
          <span data-tip="打开关于该生物群系的维基百科条目" class="icon-info-circled pointer hide"></span>
          ${
            i > 12 && !b.cells[i]
  ...
  function biomeChangeHabitability(el) {
    ...
    if (failed) {
      ...
      tip("请输入0-9999范围内的有效数字", false, "error");
      return;
    }
  ...
  function openWiki(el) {
    ...
    if (biomeName === "Custom" || !biomeName) return tip("请填写生物群系名称", false, "error");

    const wikiBase = "https://zh.wikipedia.org/wiki/";
    const pages = {
      "热沙漠": "沙漠气候#热沙漠气候",
      "冷沙漠": "沙漠气候#冷沙漠气候",
      "稀树草原": "热带和亚热带草原、稀树草原和灌丛",
      "草原": "温带草原、稀树草原和灌丛",
      "热带季雨林": "热带季雨林",
      "温带落叶林": "温带落叶阔叶林",
      "热带雨林": "热带雨林",
    ...
    const customBiomeLink = `https://zh.wikipedia.org/w/index.php?search=${biomeName}`;
  ...
  function addCustomBiome() {
    ...
    if (i > 254) {
      tip("已达到最大生物群系数目（255），需要清理数据", false, "error");
      return;
    }
    ...
    const line = `<div class="states biomes" data-id="${i}" data-name="${b.name[i]}" data-habitability=${b.habitability[i]} data-cells=0 data-area=0 data-population=0 data-color=${b.color[i]}>
      <fill-box fill="${b.color[i]}"></fill-box>
      <input data-tip="生物群系名称。点击可修改" class="biomeName" value="${b.name[i]}" autocorrect="off" spellcheck="false">
      <span data-tip="生物群系宜居度百分比" class="hide">%</span>
      <input data-tip="生物群系宜居度百分比。点击可设置新值" type="number" min=0 max=9999 step=1 class="biomeHabitability hide" value=${b.habitability[i]}>
      <span data-tip="单元格数量" class="icon-check-empty hide"></span>
      <div data-tip="单元格数量" class="biomeCells hide">${b.cells[i]}</div>
      <span data-tip="生物群系面积" style="padding-right: 4px" class="icon-map-o hide"></span>
  ...
  function downloadBiomesData() {
    ...
    let data = "ID,生物群系,颜色,宜居度,单元格数量,面积（" + unit + "）,人口\n"; // 表头
    ...
    const name = getFileName("生物群系") + ".csv";
  ...
  function enterBiomesCustomizationMode() {
    ...
    tip("点击生物群系进行选择，拖动圆圈可更改生物群系", true);
    viewbox
  ...
  function selectBiomeOnMapClick() {
    ...
    if (pack.cells.h[i] < 20) {
      tip("无法通过生物群系编辑器重新分配水域。请编辑高程图以改变水域", false, "error");
      return;
    }
