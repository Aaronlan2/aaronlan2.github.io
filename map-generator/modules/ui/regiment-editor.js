...
function editRegiment(selector) {
  ...
  function drawBase() {
    ...
    const base = viewbox
      ...
      .attr("cursor", "move");
    base
      .on("mouseenter", () => tip("军团基地。拖动以重新定位军团", true))
  ...
  function attackRegimentOnClick() {
    ...
    if (regSelected === elSelected) {
      tip("军团不能攻击自己", false, "error");
      return;
    }
    if (oldState === newState) {
      tip("不能攻击友军军团", false, "error");
      return;
    }
  ...
  function toggleAttach() {
    ...
    if (byId("regimentAttach").classList.contains("pressed")) {
      ...
      tip("点击另一个军团以合并双方军团。当前军团将被移除", true);
      armies.selectAll(":scope > g").classed("draggable", false);
    } else {
      clearMainTip();
      armies.selectAll(":scope > g").classed("draggable", true);
  ...
  function attachRegimentOnClick() {
    ...
    if (army.parentElement.id !== "armies") {
      tip("请点击要合并的军团", false, "error");
      return;
    }
    if (regSelected === elSelected) {
      tip("不能合并军团到自己。请点击另一个军团", false, "error");
      return;
    }
  ...
  function removeRegiment() {
    alertMessage.innerHTML = "确定要移除军团吗？";
    $("#alert").dialog({
      resizable: false,
      title: "移除军团",
      buttons: {
