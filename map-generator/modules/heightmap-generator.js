...
window.HeightmapGenerator = (function () {
  ...
  const fromTemplate = (graph, id) => {
    ...
    if (!steps.length) throw new Error(`高度图模板：无步骤。模板：\${id}。步骤：\${steps}`);
    setGraph(graph);

    for (const step of steps) {
      const elements = step.trim().split(" ");
      if (elements.length < 2) throw new Error(`高度图模板：步骤少于 2。模板：${id}。步骤：${elements}`);
