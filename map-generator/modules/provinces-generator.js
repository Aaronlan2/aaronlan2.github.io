...
window.Provinces = (function () {
  const forms = {
    君主制: {县: 22, 伯爵领: 6, 郡: 4, 侯爵领: 2, 男爵领: 2, 上尉辖区: 1, 总管辖区: 1},
    共和制: {省: 9, 总督辖区: 2, 区: 1, 州: 1},
    神权制: {教区: 3, 教长区: 1},
    联邦制: {省: 1, 州: 1, 共和国: 1, 县: 1, 议事会辖区: 1},
    无政府状态: {议事会辖区: 1, 公社: 1, 社区: 1, 部落领地: 1},
    荒野: {领地: 15, 大区: 2, 部落领地: 1, 氏族领地: 1, 属地: 1, 地区: 1}
  };

  const generate = (regenerate = false, regenerateLockedStates = false) => {
    TIME && console.time("生成省份");
    ...
    states.forEach(s => {
      ...
      const getColonyName = () => {
        ...
        return spliced[0] ? `新${spliced[0]}` : null;
      };
