...
function hasChinese(str) {
  return /[\u4e00-\u9fa5\u3400-\u4dbf\u4dc0-\u4dff\u9fa6-\u9fbb\u20000-\u2a6df\u2a700-\u2b73f\u2b740-\u2b81f]/.test(str);
}
...
scaleBar.on("mousemove", () => tip("单击以打开单位编辑器")).on("click", () => editUnits());
legend
...
document.addEventListener("DOMContentLoaded", async () => {
  if (!location.hostname) {
    ...
    alertMessage.innerHTML = /* html */ `Fantasy Map Generator 必须运行在服务器上，请按照 <a href="${wiki}" target="_blank">说明</a> 轻松运行本地 Web 服务器`;
...
async function checkLoadParameters() {
  ...
  if (params.get("maplink")) {
    ...
    if (valid) {
      ...
      return;
    } else showUploadErrorMessage("所给地图链接无效", maplink);
  }

  // if there is a seed (user of MFCG provided), generate map for it
  if (params.get("seed")) {
    WARN && console.warn("用种子生成地图");
...
function findBurgForMFCG(params) {
  ...
  if (pack.burgs.length < 2) {
    ERROR && console.error("无法为中世纪幻想城生成器（MFCG）选择城镇");
    return;
  }
  ...
  if (!burgId) {
    ERROR && console.error("无法为中世纪幻想城生成器（MFCG选择城镇");
    return;
  }
  ...
  invokeActiveZooming();
  tip("这里矗立着辉煌的城市 —— " + b.name, true, "success", 15000);
}
...
async function generate(options) {
  try {
    ...
    WARN && console.warn(`共计: ${rn((performance.now() - timeStart) / 1000, 2)}s`);
    showStatistics();
...
function showStatistics() {
  ...
  const stats = `  种子: ${seed}
    画布尺寸: ${graphWidth}x${graphHeight} px
    高程图: ${heightmap}
    模板: ${isRandomTemplate}${heightmapType}
    生成点数: ${grid.points.length}
    网格数: ${pack.cells.i.length}
    地图大小: ${mapSizeOutput.value}%
    国家数量: ${pack.states.length - 1}
    省份数量: ${pack.provinces.length - 1}
    城镇数量: ${pack.burgs.length - 1}
    宗教数量: ${pack.religions.length - 1}
    文化背景: ${culturesSet.value}
    文化数量: ${pack.cultures.length - 1}`;
